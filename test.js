...
function extractKeys(data, keys = new Set()) {
  if (Array.isArray(data)) {
    ...
    Object.keys(data).forEach((key) => {
      keys.add(key);
      extractKeys(data[key], keys);
    });
  }
  return keys;
}

const keys = extractKeys(basicData);
console.log(keys);

// "교통"이 포함된 값의 합계와 언급 횟수를 계산하는 함수
function calculateTransportInfo(data) {
  let totalCost = 0; // 교통비 합계
  let transportCount = 0; // 교통 키 언급 횟수
  ...
  return { totalCost, transportCount };
}

// 함수 호출
const { totalCost, transportCount } = calculateTransportInfo(basicData);

// 결과 출력
console.log(`'교통' 키에 해당하는 값의 합계: ${totalCost}`);
console.log(`'교통' 키가 언급된 횟수: ${transportCount}`);
